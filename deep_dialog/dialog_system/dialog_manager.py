...
class DialogManager:
    ...
    def __init__(self, agent, user, act_set, slot_set):
        self.agent = agent
        self.user = user
    ...
    def initialize_episode(self):
        ...
        self.possible_answer = self.state_tracker.get_suggest_slots_values(self.user_action['request_slots'])
    ...
    def next_turn(self, record_training_data=True):
        ...
        return (self.episode_over, self.reward, self.agent_action['act_slot_response'], self.user_action)
    ...
    def print_function(self, agent_action=None, user_action=None):
        ...
        if agent_action:
            ...
            if self.agent.__class__.__name__ == 'AgentCmd': # command line agent
                ...
                #if 'serial_no'in user_request_slots.keys(): del user_request_slots['serial_no']
                if len(user_request_slots) > 0:
                    ...
                    print ('(Number of Course in KB satisfying current constraints: %s)' % len(kb_results))
