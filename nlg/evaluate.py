...
def word2char(inputs):
    char_inputs = []
    for line in inputs:
        char_inputs.append(list(''.join(line)))
    return char_inputs
...
def get_bleu(hypotheses, reference):
    ...
    hypotheses = word2char(hypotheses)
    reference = word2char(reference)
...
def decode_minibatch(
    ...
    for i in range(config['data']['max_trg_length']):
        ...
        decoder_argmax = word_probs.data.numpy().argmax(axis=-1)
        next_preds = Variable(
...
def evaluate_autoencode_model(
    ...
    for j in range(0, len(src_test['data']), config['data']['batch_size']):
        ...
        output_lines_trg_gold = input_lines_src.data.numpy()
        output_lines_trg_gold = [
            [src['id2word'][x] for x in line]
            for line in output_lines_trg_gold
        ]
